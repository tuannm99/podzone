...
export function Paragraph(props: ParentProps<{ muted?: boolean; class?: string }>) {
    return (
        <p class={classes('text-base leading-7', props.muted ? 'text-gray-500' : 'text-gray-700', props.class)}>
            {props.children}
        </p>
    )
}
...
export function Blockquote(props: ParentProps<{ cite?: string; class?: string }>) {
    return (
        <figure class={classes('rounded-lg border-s-4 border-gray-900 bg-gray-50 px-5 py-4', props.class)}>
            <blockquote class="text-base leading-7 text-gray-700">{props.children}</blockquote>
            {props.cite ? <figcaption class="mt-3 text-sm font-medium text-gray-500">{props.cite}</figcaption> : null}
        </figure>
    )
}
...
export function InlineCode(props: ParentProps<{ class?: string }>) {
    return (
        <code class={classes('rounded-md bg-gray-100 px-1.5 py-0.5 text-sm text-gray-800', props.class)}>
            {props.children}
        </code>
    )
}
...
export function Divider(props: { label?: string; class?: string }) {
    return (
        <div class={classes('flex items-center gap-4', props.class)}>
            ...
            {props.label ? (
                <>
                    <span class="text-xs font-semibold uppercase tracking-[0.2em] text-gray-400">{props.label}</span>
                    <div class="h-px flex-1 bg-gray-200" />
                </>
            ) : null}
        </div>
...
export function ProseList(props: { items: Array<string | JSX.Element>; ordered?: boolean; class?: string }) {
