...
export function Calendar(props: {
    ...
    return (
        <div class={classes('rounded-lg border border-gray-200 bg-white p-4 shadow-xl', props.class)}>
            <div class="flex items-center justify-between gap-3">
                ...
                <p class="text-sm font-semibold text-gray-900">{formatMonthYear(cursor())}</p>
                <button
                    type="button"
                    class="rounded-md px-3 py-2 text-sm font-medium text-gray-600 transition hover:bg-gray-100 hover:text-gray-900"
                    onClick={() => setCursor((date) => addMonths(date, 1))}
            ...
            <div class="mt-4 flex items-center justify-between gap-3 border-t border-gray-100 pt-4">
                ...
                <Show when={props.value}>
                    <span class="text-sm text-gray-500">{formatDisplayValue(props.value, '')}</span>
                </Show>
            </div>
        </div>
    )
...
export function DatepickerField(props: {
    ...
    return (
        <FieldLabel label={props.label}>
            <div class={classes('relative', props.class)} ref={(element) => (container = element)}>
                <button
                    type="button"
                ...
                >
                    <span class={props.value ? 'text-gray-900' : 'text-gray-400'}>
                        {formatDisplayValue(props.value, props.placeholder ?? 'Select a date')}
                    </span>
