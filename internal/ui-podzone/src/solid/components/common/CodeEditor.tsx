...
export function CodeEditor(props: CodeEditorProps) {
    ...
    const lineCount = createMemo(() => (props.value === '' ? 1 : props.value.split('\n').length))
    const minHeight = createMemo(() => `${Math.max(props.rows ?? 22, 16) * 24 + 32}px`)
    ...
    createEffect(() => {
        ...
        currentView.dispatch({
            effects: languageCompartment.reconfigure(resolveLanguageExtension(props.language)),
        })
    })
    ...
    return (
        <div class={classes('space-y-3', props.class)}>
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div class="space-y-1">
                    <div class="text-sm font-medium text-gray-800">{props.label ?? 'Code'}</div>
            ...
            <div class="overflow-hidden rounded-lg bg-white shadow-sm">
                <div class="flex items-center justify-between border border-slate-200 border-b-0 bg-white/90 px-4 py-2 text-xs text-slate-500">
                    <span class="font-medium uppercase tracking-[0.18em] text-slate-600">Editor</span>
            ...
            <Show when={mode() === 'normal'}>
                <div class="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                    Normal mode is active. Press <kbd class="rounded bg-white px-1.5 py-0.5 font-mono">i</kbd> to type.
                </div>
            </Show>
        </div>
    )
