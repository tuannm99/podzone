...
export function useOrderInsights(params: OrderInsightsParams) {
      ...
      case 'my_queue':
        return (
          ...
          order.operatorAssignee.toLowerCase() ===
            params.operatorLens().trim().toLowerCase()
        )
      case 'overdue':
        return (
  ...
  const filteredOrders = () =>
    params
      .orders()
      .filter((order) => matchesQueueView(order, params.activeQueueView()))
  ...
  const queueViewCount = (view: QueueView) =>
    params.orders().filter((order) => matchesQueueView(order, view)).length

  const blockedOrders = () =>
    params.orders().filter((order) => order.status === 'routing_blocked')
  ...
  const copyStoreActivityFeed = async () => {
    ...
    try {
      await navigator.clipboard.writeText(summary)
      params.setMessage(
        `Copied store activity feed for ${params.storeLabel()}.`
      )
    } catch {
