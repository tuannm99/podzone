                            ...
                            <TableRow>
                                <TableCell>
                                    <input
                                        ...
                                        onChange={(event) =>
                                            props.toggleSelected(order.id, event.currentTarget.checked)
                                        }
                                    />
                                </TableCell>
                                <TableCell>
                                    <p class="font-semibold text-gray-900">{order.productTitle}</p>
                                    <p class="mt-1 max-w-44 truncate text-xs text-gray-500">{order.id}</p>
                                    <p class="mt-1 text-xs text-gray-500">
                                        {order.customerName} · {order.quantity} item
                                        {order.quantity === 1 ? '' : 's'}
                                    </p>
                                </TableCell>
                                <TableCell class="text-gray-700">{order.partner || 'Unassigned'}</TableCell>
                                <TableCell>
                                    <Badge content={String(props.priorityScoreFor(order))} color="indigo" />
                                </TableCell>
                                <TableCell>
