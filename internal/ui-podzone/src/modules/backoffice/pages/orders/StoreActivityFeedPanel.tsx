    ...
    return (
        <div class="rounded-lg border border-slate-200 bg-slate-50 p-4">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <p class="text-sm font-semibold text-slate-900">Store activity feed</p>
                    <p class="text-sm text-slate-500">
                        Latest activity across the current queue slice for store {insights.storeLabel()}.
                    </p>
                </div>
                <Button
                    type="button"
                ...
                <Button type="button" size="xs" color="alternative" href={workspaceURL('/orders/audit')}>
                    Open full audit
                </Button>
                <Button type="button" size="xs" color="alternative" href={workspaceURL('/orders/finance')}>
                    Open finance view
                </Button>
            </div>
                            ...
                            <div class="rounded-md border border-slate-200 bg-white p-3">
                                <div class="flex flex-wrap items-center justify-between gap-2">
                                    <div class="flex flex-wrap items-center gap-2">
                                        ...
                                        />
                                        <p class="text-xs font-semibold text-slate-700">{entry.orderId}</p>
                                        <p class="text-xs text-slate-500">{entry.productTitle}</p>
                                    </div>
                                    <p class="text-xs text-slate-500">{formatActivityTime(entry.activity.createdAt)}</p>
                                </div>
                                <div class="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-500">
                                    <span>{formatActivityActor(entry.activity.actor)}</span>
                                    <span>owner {entry.operatorAssignee || 'unassigned'}</span>
                                </div>
                                <p class="mt-2 text-sm text-slate-700">{entry.activity.message}</p>
