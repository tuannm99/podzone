...
export function createOrderFinanceViewModel(
  options: OrderFinanceViewModelOptions
) {
  ...
  const financeOrders = () =>
    [...orders()].filter(hasFinanceAttention).sort((left, right) => {
      const disputedOrder =
        Number(left.settlementStatus !== 'disputed') -
        Number(right.settlementStatus !== 'disputed')
      return (
        disputedOrder ||
