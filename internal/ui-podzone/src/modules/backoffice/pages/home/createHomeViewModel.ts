...
export function createHomeViewModel(options: HomeViewModelOptions) {
  ...
  const [topPartnerLoad, setTopPartnerLoad] = createSignal(
    'No partner load yet'
  )
  ...
  const loadSnapshot = async () => {
    ...
    setMockRevenue(
      formatMoney(
        orders.reduce((sum, order) => sum + parseMoney(order.total), 0)
      )
    )
    setRealizedMarginTotal(
      formatMoney(
        orders.reduce((sum, order) => sum + parseMoney(order.realizedMargin), 0)
      )
    )
    ...
    setDisputedSettlementCount(
      orders.filter((order) => order.settlementStatus === 'disputed').length
    )
    setIssueCostExposure(
      formatMoney(
        orders.reduce((sum, order) => sum + parseMoney(order.issueCost), 0)
      )
    )
    ...
    const loadByPartner = orders.reduce<Record<string, number>>(
      (result, order) => {
    ...
    setTopPartnerLoad(
      topPartner
        ? `${topPartner[0]} · ${topPartner[1]} orders`
        : 'No partner load yet'
    )
    setIssueRate(
      orders.length > 0
        ? `${Math.round((openIssues.length / orders.length) * 100)}%`
        : '0%'
    )
  }
