...
export function createAdminIamViewModel() {
    ...
    const policyContextValue = {
        ...
        policyScopeOptions: () =>
            state.canManagePlatform()
                ? policyScopeOptions
                : policyScopeOptions.filter((option) => option.value === 'organization'),
    ...
    const groupContextValue = {
        ...
        searchMemberUsers: (search: string) => {
            if (state.groupScope() === 'platform') return state.searchPlatformUsers(search)
            if (state.groupScope() === 'tenant') return state.searchTenantUsers(search, state.groupTenantId())
    ...
    const principalContextValue = {
        permissionOptions: state.permissionOptions,
        managedPolicyOptions: () => state.managedPolicyOptions(state.principalMode()),
        ...
        searchTenantUsers: (search: string) => state.searchTenantUsers(search, state.principalTenantId()),
        ...
        attachPrincipalManagedPolicyFromForm: actions.attachPrincipalManagedPolicyFromForm,
        handleAttachPrincipalManagedPolicy: actions.handleAttachPrincipalManagedPolicy,
        ...
        handleDetachPrincipalManagedPolicy: actions.handleDetachPrincipalManagedPolicy,
        ...
        currentBoundary: () =>
            state.principalMode() === 'platform' ? state.platformUserBoundary() : state.tenantUserBoundary(),
        ...
        setPrincipalInlinePolicyDescription: state.setPrincipalInlinePolicyDescription,
        principalInlinePolicyJson: state.principalInlinePolicyJson,
        setPrincipalInlinePolicyJson: state.setPrincipalInlinePolicyJson,
        savePrincipalInlinePolicyFromForm: actions.savePrincipalInlinePolicyFromForm,
        ...
        handleDeletePrincipalInlinePolicy: actions.handleDeletePrincipalInlinePolicy,
    }

    const trustSimContextValue = {
        permissionOptions: state.permissionOptions,
        boundaryPolicyOptions: () =>
            state.managedPolicyOptions(state.trustRoleName().startsWith('tenant_') ? 'tenant' : 'platform'),
        platformUserOptions: state.platformUserOptions,
        platformUsersLoading: state.platformUsersLoading,
        platformUsersError: state.platformUsersError,
        searchPlatformUsers: state.searchPlatformUsers,
        ...
        tenantUsersLoading: state.tenantUsersLoading,
        tenantUsersError: state.tenantUsersError,
        searchTenantUsers: (search: string) => state.searchTenantUsers(search, state.simTenantId()),
    ...
    return {
        ...
        assignments: {
            ...
            tenantUsersLoading: state.tenantUsersLoading,
            tenantUsersError: state.tenantUsersError,
            searchTenantUsers: (search: string) => state.searchTenantUsers(search, state.shortcutTenantId()),
