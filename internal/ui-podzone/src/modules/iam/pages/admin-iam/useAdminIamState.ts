...
export function useAdminIamState(userID: number) {
  ...
  const [policyVersions, setPolicyVersions] =
    createSignal<PolicyVersionInfo[]>([]);
  const [policyAttachments, setPolicyAttachments] =
    createSignal<PolicyAttachmentInfo[]>([]);
  ...
  const [groupInlinePolicies, setGroupInlinePolicies] =
    createSignal<GroupInlinePolicy[]>([]);
  const [roleBoundary, setRoleBoundary] =
    createSignal<RolePermissionBoundary | null>(null);
  const [platformUserPolicies, setPlatformUserPolicies] =
    createSignal<PolicyInfo[]>([]);
  const [tenantUserPolicies, setTenantUserPolicies] =
    createSignal<PolicyInfo[]>([]);
  const [platformUserInlinePolicies, setPlatformUserInlinePolicies] =
    createSignal<UserInlinePolicy[]>([]);
  const [tenantUserInlinePolicies, setTenantUserInlinePolicies] =
    createSignal<UserInlinePolicy[]>([]);
  ...
  const [policyStatementsJson, setPolicyStatementsJson] = createSignal(
    prettyJSON([{ effect: 'allow', actionPattern: 'order:read', resourcePattern: '*', conditions: [] }])
  );
  const [policyVersionJson, setPolicyVersionJson] = createSignal(
    prettyJSON([{ effect: 'allow', actionPattern: 'order:update', resourcePattern: '*', conditions: [] }])
  );
  ...
  const [trustBoundaryPolicyName, setTrustBoundaryPolicyName] =
    createSignal('');
  ...
  const [simAssumedRoleServicePrincipal, setSimAssumedRoleServicePrincipal] =
    createSignal('');
  const [simAssumedRoleExpiresAt, setSimAssumedRoleExpiresAt] =
    createSignal('');
  const [principalMode, setPrincipalMode] =
    createSignal<'platform' | 'tenant'>('platform');
  ...
  const [principalInlinePolicyName, setPrincipalInlinePolicyName] =
    createSignal('');
  const [principalInlinePolicyDescription, setPrincipalInlinePolicyDescription] =
    createSignal('');
  ...
  return {
    pageError, setPageError, pageMessage, setPageMessage, loading, setLoading,
    allowed, setAllowed, memberships, setMemberships, organizations,
    setOrganizations, policies, setPolicies, groups, setGroups, selectedOrgId,
    ...
    orgPolicies, setOrgPolicies, groupMembers, setGroupMembers, groupPolicies,
    setGroupPolicies, groupInlinePolicies, setGroupInlinePolicies, roleBoundary,
    setRoleBoundary, platformUserPolicies, setPlatformUserPolicies,
    tenantUserPolicies, setTenantUserPolicies, platformUserInlinePolicies,
    setPlatformUserInlinePolicies, tenantUserInlinePolicies,
    setTenantUserInlinePolicies, platformUserBoundary, setPlatformUserBoundary,
    tenantUserBoundary, setTenantUserBoundary, simulation, setSimulation,
...
export type AdminIamState = ReturnType<typeof useAdminIamState>;
