...
export function createDirectoryViewModel(params: {
    ...
    createPaginatedResource(
      ...
      },
      {
        enabled: () =>
          params.allowed() &&
          (scope === 'organization'
            ? Boolean(params.selectedOrgId().trim())
            : params.canManagePlatform()),
  ...
  const [visibleIdentities] = createResource(
    visibleIdentityRequest,
    async (request) => {
  ...
  return {
    ...
    managedPolicyOptions: (scope: string) => {
      const policies =
        policyDirectory[scope as keyof typeof policyDirectory] ||
        platformPolicies
      return [
