...
export function createAssignmentsActions(
  state: AdminIamState,
  runAction: RunAction
) {
  const platformUserID = () =>
    Number.parseInt(state.shortcutPlatformUserId().trim(), 10)
  const tenantUserID = () =>
    Number.parseInt(state.shortcutTenantUserId().trim(), 10)
  ...
  const handleAssignPlatformRole = () =>
    runAction(async () => {
      ...
      state.setPageMessage(
        `Assigned platform role ${roleName} to user ${userID}.`
      )
    })
  ...
  const handleRemovePlatformRoleShortcut = () =>
    runAction(async () => {
      ...
      if (!result.success) throw new Error(result.message)
      state.setPageMessage(
        `Removed platform role ${roleName} from user ${userID}.`
      )
    })
  ...
  const handleAssignTenantRole = () =>
    runAction(async () => {
      ...
      if (!result.success) throw new Error(result.message)
      state.setPageMessage(
        `Assigned tenant role ${roleName} to user ${userID}.`
      )
    })
