...
import { createSimulationPresets } from './trust-simulation/simulation-presets'
import type {
  PrincipalBoundaryFormValues,
  PrincipalInlinePolicyFormValues,
  PrincipalManagedPolicyFormValues,
} from './principals/forms'
...
export function createAdminIamPrincipalTrustActions(
  state: AdminIamState,
  ...
  const presets = createSimulationPresets(state)
  ...
  const handleSimulate = async () => {
    await runAction(async () => {
      ...
      const result = await simulateAccess({
        ...
        sessionTags: parseJSONObject(
          state.simSessionTagsJson(),
          'Session tags'
        ),
      })
      if (!result.success) throw new Error(result.message)
  ...
  const handleRemoveTenantMembershipShortcut = async () => {
    await runAction(async () => {
      ...
      if (!result.success) throw new Error(result.message)
      state.setPageMessage(
        `Removed tenant membership for user ${targetUserId}.`
      )
    })
  }
