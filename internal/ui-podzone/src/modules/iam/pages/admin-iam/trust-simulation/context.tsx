...
export type AdminIamTrustSimContextValue = {
  ...
  simulationSourceColor: (
    source: string
  ) => 'blue' | 'green' | 'yellow' | 'pink' | 'dark' | 'red' | 'indigo'
...
export function AdminIamTrustSimProvider(
  props: ParentProps<{ value: AdminIamTrustSimContextValue }>
) {
  return (
    <AdminIamTrustSimContext.Provider value={props.value}>
      {props.children}
    </AdminIamTrustSimContext.Provider>
  )
}
