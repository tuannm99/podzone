...
export function IamKeyValueBuilder(props: {
    ...
    const updateEntry = (index: number, patch: Partial<KeyValueEntry>) => {
        commit(entries().map((entry, currentIndex) => (currentIndex === index ? { ...entry, ...patch } : entry)))
    }
    ...
    return (
        <div class="space-y-3">
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <Badge content={`${count()} ${props.badgeLabel || 'entries'}`} color="blue" />
            </div>
                        ...
                        {(entry, entryIndex) => (
                            <Card class="space-y-3 border border-gray-200 bg-gray-50 p-4 shadow-none">
                                <div class="flex items-center justify-between gap-3">
                                    ...
                                    <Button size="xs" color="red" onClick={() => removeEntry(entryIndex())}>
                                        Remove
                                    </Button>
                                </div>
