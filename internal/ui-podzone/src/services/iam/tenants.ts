...
export async function listUserTenants(userId: number): Promise<IamResult<TenantMembership[]>> {
    try {
        const { data } = await http.get<{ memberships?: TenantMembership[] }>(`/auth/v1/iam/users/${userId}/tenants`)
...
export async function createTenant(payload: CreateTenantPayload): Promise<IamResult<CreateTenantResult>> {
    try {
        const { data } = await http.post<CreateTenantResult>('/auth/v1/iam/tenants', payload)
...
export async function upsertTenantMember(payload: UpsertTenantMemberPayload): Promise<IamResult<true>> {
    try {
...
export async function upsertTenantMemberByIdentity(
    ...
    try {
        ...
        return toFailure(error as HttpError, 'Failed to save tenant member by identity')
    }
}

export async function removeTenantMember(tenantId: string, userId: number): Promise<IamResult<true>> {
    try {
...
): Promise<IamResult<{ invite?: TenantInvite; inviteToken: string; acceptUrl: string }>> {
    try {
        const { data } = await http.post<{
            invite?: TenantInvite
            inviteToken?: string
...
export async function revokeTenantInvite(inviteId: string): Promise<IamResult<true>> {
    try {
...
export async function acceptTenantInvite(inviteToken: string): Promise<IamResult<TenantMembership>> {
    try {
        const { data } = await http.post<{ membership?: TenantMembership }>('/auth/v1/iam/invites:accept', {
            inviteToken,
        })
        if (!data.membership) {
            return {
