...
func newTestOrderRoutingHarness() *testOrderRoutingHarness {
	return &testOrderRoutingHarness{orders: map[string]routingentity.RoutedOrder{}}
}
...
func newOrderRoutingTestInteractor(
		...
		RunAndReturn(func(_ context.Context, query inputport.ListRoutedOrderActivitiesQuery) (*routingentity.RoutedOrderActivityFeedPage, error) {
			...
			for _, order := range orderState.orders {
				...
				for _, activity := range order.ActivityLog {
					if query.ActivityType == "notes" && activity.Type == routingentity.RoutedOrderActivityTypeSystem {
						continue
					}
					...
					if !query.IncludeSystem && query.ActivityType != "system" && query.ActivityType != "notes" &&
						activity.Type == routingentity.RoutedOrderActivityTypeSystem {
						continue
					}
					...
					entries = append(entries, routingentity.RoutedOrderActivityFeedEntry{
						OrderID:          order.ID,
	...
	ordersMock.EXPECT().
		...
		RunAndReturn(func(_ context.Context, id string) (*routingentity.RoutedOrder, error) {
	...
	ordersMock.EXPECT().
		...
		RunAndReturn(func(_ context.Context, order routingentity.RoutedOrder) (*routingentity.RoutedOrder, error) {
	...
	partnersMock.EXPECT().
		...
		RunAndReturn(func(context.Context, string) ([]routingentity.PartnerRoutingProfile, error) {
			return append([]routingentity.PartnerRoutingProfile(nil), partnerState...), nil
		}).
		Maybe()

	return NewOrderRoutingInteractor(ordersMock, productsMock, partnersMock), orderState, productsMock, partnersMock
}

func cloneOrder(order routingentity.RoutedOrder) routingentity.RoutedOrder {
	cloned := order
	cloned.Timeline = append([]string(nil), order.Timeline...)
	cloned.ActivityLog = append([]routingentity.RoutedOrderActivity(nil), order.ActivityLog...)
...
func encodeTestActivityCursor(entry routingentity.RoutedOrderActivityFeedEntry) string {
