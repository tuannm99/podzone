...
func TestCreateRoutedOrderMapsInputAndOutput(t *testing.T) {
	...
	orderUC.EXPECT().
		...
		RunAndReturn(func(_ context.Context, cmd backofficeoperations.CreateRoutedOrderCmd) (*routingctx.RoutedOrder, error) {
			require.Equal(t, backofficeoperations.CreateRoutedOrderCmd{
				StoreID:          testStoreID,
				CandidateID:      "cand-1",
	...
	require.Equal(t, routingctx.RoutedOrderSettlementStatusPending, got.SettlementStatus)
}
...
func TestBulkUpdateRoutedOrdersMapsPointersAndList(t *testing.T) {
	...
	owner := "ops.lead"
	status := routingctx.RoutedOrderSettlementStatusPaid
	orderUC := operationsmocks.NewMockOrderRoutingUsecase(t)
	productUC := cataloginputmocks.NewMockProductSetupUsecase(t)
	orderUC.EXPECT().
		...
		RunAndReturn(func(
			_ context.Context,
			cmd backofficeoperations.BulkUpdateRoutedOrdersCmd,
...
func TestForceRerouteBlockedOrderMapsInputAndOutput(t *testing.T) {
	...
	orderUC.EXPECT().
		...
		RunAndReturn(func(_ context.Context, cmd routingctx.ForceRerouteBlockedOrderCmd) (*routingctx.RoutedOrder, error) {
			require.Equal(t, routingctx.ForceRerouteBlockedOrderCmd{
				StoreID:          testStoreID,
				OrderID:          "ord-1",
	...
	require.Equal(t, routingctx.RoutedOrderStatusQueued, got.Status)
}
		...
		Drafts: []catalogentity.ProductSetupDraft{
			{
				...
				BaseCost:    "$8.00",
				RetailPrice: "$20.00",
				Status:      catalogentity.ProductSetupDraftStatusReadyForReview,
				Notes:       "ready",
				CreatedAt:   now,
				UpdatedAt:   now,
			},
...
func TestUpdateOrderShipmentPropagatesUsecaseErrors(t *testing.T) {
	...
	orderUC.EXPECT().
		...
		RunAndReturn(func(
			_ context.Context,
			cmd backofficeoperations.UpdateOrderShipmentCmd,
		) (*routingctx.RoutedOrder, error) {
	...
	orderUC.EXPECT().
		...
		RunAndReturn(func(
			...
			return &routingctx.RoutedOrderActivityFeedPage{
				Entries: []routingctx.RoutedOrderActivityFeedEntry{
					{
						OrderID:          "ord-1",
						ProductTitle:     "Vintage Tee",
						Partner:          "Print Partner A",
						...
						Activity: routingctx.RoutedOrderActivity{
							Type:      routingctx.RoutedOrderActivityTypeShipmentNote,
							Actor:     "user:12",
							Message:   "Handed off to carrier",
							Details:   []routingctx.RoutedOrderActivityDetail{{Key: "carrier", Value: "DHL"}},
							CreatedAt: since,
						},
					},
				},
...
func TestRoutedOrderRecommendationMapsQueryAndResponse(t *testing.T) {
	...
	orderUC.EXPECT().
		...
		RunAndReturn(func(
			...
			require.Equal(t, routingctx.RecommendRoutedOrderPartnerQuery{
				StoreID:          testStoreID,
				CandidateID:      "cand-1",
				ProductType:      "tshirt",
				ShipRegion:       "us",
			...
			return &routingctx.RoutedOrderRecommendation{
				CandidateID:       "cand-1",
				ProductTitle:      "Vintage Tee",
				...
				Options: []routingctx.RoutingPartnerOption{
					{
						Partner: routingctx.PartnerRoutingProfile{
							ID:                    "prt-2",
							Code:                  "fulfill-fast",
							Name:                  "Fulfill Fast",
	...
	require.Empty(t, got.BlockedReasonCode)
