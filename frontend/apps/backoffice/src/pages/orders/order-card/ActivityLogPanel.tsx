    ...
    return (
        <div class="mt-3 rounded-md border border-slate-200 bg-white p-3">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <p class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-600">Activity log</p>
                <div class="flex flex-wrap items-center gap-2">
                    <div class="min-w-[11rem]">
                        <SelectField
                            ...
                            onChange={(event) =>
                                props.ui.setActivityFilter(event.currentTarget.value as ActivityFilter)
                            }
                        />
                    </div>
                ...
                <Show
                    ...
                    fallback={
                        <div class="rounded-md border border-dashed border-slate-200 bg-slate-50 p-3 text-sm text-slate-500">
                            <Show
                                when={props.helpers.hiddenSystemActivityCountFor(props.order) > 0}
                                fallback={'No activity matches the current filter.'}
                            >
                                {props.helpers.hiddenSystemActivityCountFor(props.order)} system updates are hidden.
                            </Show>
                        </div>
                    }
                >
                        ...
                        {(activity) => (
                            <div class="rounded-md border border-slate-200 bg-slate-50 p-3">
                                <div class="flex flex-wrap items-center justify-between gap-2">
                                    ...
                                    <p class="text-xs text-slate-500">{formatActivityTime(activity.createdAt)}</p>
                                </div>
