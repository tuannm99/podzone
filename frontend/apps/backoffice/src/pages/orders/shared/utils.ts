...
export function formatActivitySummary(order: RoutedOrder, activities: RoutedOrder['activityLog']) {
    const header = [
        `Order ${order.id}`,
    ...
    const activityLines = activities.map((activity) => {
        const details = activity.details.map((detail) => `${detail.key}=${detail.value}`).join(', ')
        return [
...
export function formatStoreActivitySummary(tenantId: string, entries: StoreActivityFeedEntry[]) {
