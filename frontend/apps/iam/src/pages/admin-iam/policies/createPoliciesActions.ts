...
export function createPoliciesActions(state: AdminIamState, loaders: AdminIamLoaders, runAction: RunAction) {
    const createPolicyFromForm = (values: CreatePolicyFormValues) =>
        runAction(async () => {
            const statements = parseJSONArray<PolicyStatement>(values.statementsJson, 'Policy statements')
            const result = await createPolicy({
                scope: values.scope,
                orgId: values.scope === 'organization' ? state.selectedOrgId().trim() : undefined,
    ...
    const createPolicyVersionFromForm = (values: CreatePolicyVersionFormValues) =>
        runAction(async () => {
            const statements = parseJSONArray<PolicyStatement>(values.statementsJson, 'Policy version statements')
            ...
            state.setPageMessage(`Created a new version for ${state.selectedPolicyName().trim()}.`)
    ...
    const handleSetDefaultVersion = (version: string) =>
        runAction(async () => {
            ...
            const result = await setDefaultPolicyVersion(state.policyRef(policyName), version)
            if (!result.success) throw new Error(result.message)
