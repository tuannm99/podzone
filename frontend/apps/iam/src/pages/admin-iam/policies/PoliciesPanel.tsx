        ...
        <>
            ...
            <form class="space-y-3" onSubmit={submitCreatePolicy}>
                <div class="grid gap-3 md:grid-cols-2">
                    ...
                    />
                    <FormInputField form={createPolicyForm} name="name" label="Policy name" />
                </div>
                <FormInputField form={createPolicyForm} name="description" label="Description" />
                <IamStatementBuilder
                    label="Statements"
                    actionOptions={policy.permissionOptions()}
                    value={createPolicyForm.values.statementsJson}
                    onChange={(value) => createPolicyForm.setValue('statementsJson', value)}
                />
                <Show when={createPolicyForm.hasError('statementsJson')}>
                    <p class="text-xs font-medium text-red-600">{createPolicyForm.error('statementsJson')}</p>
                </Show>
                <Button type="submit" size="sm" loading={createPolicyForm.isSubmitting()}>
                    Create policy
                </Button>
            </form>
                ...
                {(detail) => (
                    <div class="rounded-lg bg-gray-50 p-4 text-sm text-gray-600">
                        ...
                        <div class="mt-3 flex flex-wrap gap-2">
                            ...
                            <Badge content={`default ${detail().defaultVersion || 'v1'}`} color="green" />
            ...
            <Show when={versionForm.hasError('statementsJson')}>
                <p class="text-xs font-medium text-red-600">{versionForm.error('statementsJson')}</p>
            </Show>
