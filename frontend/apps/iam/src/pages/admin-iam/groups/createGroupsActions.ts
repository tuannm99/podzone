...
export function createGroupsActions(state: AdminIamState, loaders: AdminIamLoaders, runAction: RunAction) {
    ...
    const createGroupFromForm = (values: CreateGroupFormValues) =>
        runAction(async () => {
            const result = await createGroup({
                ...
                tenantId: values.scope === 'tenant' ? values.tenantId.trim() : undefined,
    ...
    const handleAddGroupMember = () => addGroupMemberFromForm({ userId: state.groupMemberUserId() })
    ...
    const handleAttachGroupPolicy = () => attachGroupPolicyFromForm({ policyName: state.groupPolicyName() })
    ...
    const saveGroupInlinePolicyFromForm = (values: GroupInlinePolicyFormValues) =>
        runAction(async () => {
            const statements = parseJSONArray<PolicyStatement>(values.statementsJson, 'Group inline policy')
