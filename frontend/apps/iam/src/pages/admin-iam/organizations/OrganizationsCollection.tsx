    ...
    return (
        <div class="space-y-4">
            ...
            <Show
                ...
                fallback={
                    <EmptyBlock title="No organizations" copy="No organizations match the current collection query." />
                }
            >
                <DataTable>
                    <TableHead>
                    ...
                    <TableBody>
                        <For each={props.organizations()}>
                            {(organization) => (
                                <TableRow>
                                    <TableCell class="font-semibold text-gray-900">{organization.name}</TableCell>
                                    <TableCell class="text-gray-600">{organization.slug}</TableCell>
                                    <TableCell>
                                        <Badge
                                            content={
                                                organization.id === props.selectedOrgId() ? 'selected' : 'organization'
                                            }
                                            color={organization.id === props.selectedOrgId() ? 'blue' : 'dark'}
                                        />
                                    </TableCell>
                                    <Show when={props.canManagePlatform()}>
                                        <TableCell>
                                            <div class="flex flex-wrap justify-end gap-2">
                                                ...
                                                >
                                                    <Button
                                                        ...
                                                        color="light"
                                                        onClick={() =>
                                                            props.handleDetachTenantFromOrg(props.orgTenantId().trim())
                                                        }
                                                    >
