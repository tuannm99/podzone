...
export function createPrincipalsActions(state: AdminIamState, loaders: AdminIamLoaders, runAction: RunAction) {
    const userID = () =>
        Number.parseInt(
    ...
    const attachPrincipalManagedPolicyFromForm = (values: PrincipalManagedPolicyFormValues) =>
        runAction(async () => {
            const policyName = values.policyName.trim()
            const result =
                state.principalMode() === 'platform'
                    ? await attachPlatformUserPolicy(userID(), policyName)
                    : await attachTenantUserPolicy(state.principalTenantId().trim(), userID(), policyName)
        ...
        runAction(async () => {
            const result =
                state.principalMode() === 'platform'
                    ...
                    : await detachTenantUserPolicy(state.principalTenantId().trim(), userID(), policyName)
            if (!result.success) throw new Error(result.message)
    ...
    const savePrincipalInlinePolicyFromForm = (values: PrincipalInlinePolicyFormValues) =>
        runAction(async () => {
            const statements = parseJSONArray<PolicyStatement>(values.statementsJson, 'Inline policy')
            const result =
                state.principalMode() === 'platform'
        ...
        runAction(async () => {
            const result =
                state.principalMode() === 'platform'
                    ...
                    : await deleteTenantUserInlinePolicy(state.principalTenantId().trim(), userID(), name)
            if (!result.success) throw new Error(result.message)
        ...
        runAction(async () => {
            ...
            const result =
                state.principalMode() === 'platform'
                    ...
                    : await putTenantUserPermissionBoundary(state.principalTenantId().trim(), userID(), policyName)
            if (!result.success) throw new Error(result.message)
        ...
        runAction(async () => {
            const result =
                state.principalMode() === 'platform'
                    ...
                    : await deleteTenantUserPermissionBoundary(state.principalTenantId().trim(), userID())
            if (!result.success) throw new Error(result.message)
