...
export function RoleAssignmentsPanel(props: RoleAssignmentsPanelProps) {
    return (
        <div class="space-y-5">
            <SectionTitle title="Role assignments" subtitle="Platform roles and workspace memberships." />
            <div class="grid gap-6" classList={{ 'lg:grid-cols-2': props.canManagePlatform() }}>
                <Show when={props.canManagePlatform()}>
                    <section class="space-y-3 border-b border-gray-200 pb-6 lg:border-b-0 lg:border-r lg:pb-0 lg:pr-6">
                        ...
                        <SelectField
                            ...
                            onChange={(event) => props.setShortcutPlatformRoleName(event.currentTarget.value)}
                        />
                ...
                <section class="space-y-3">
                    <p class="text-sm font-semibold text-gray-900">Workspace membership</p>
                    <SelectField
                        label="Workspace"
                        value={props.shortcutTenantId()}
                        options={props.tenantOptions()}
                        onChange={(event) => props.setShortcutTenantId(event.currentTarget.value)}
                    />
                    <SearchSelectField
                        label="User"
                    ...
                    <SelectField
                        ...
                        onChange={(event) => props.setShortcutTenantRoleName(event.currentTarget.value)}
                    />
                    <div class="flex flex-wrap gap-3">
                        <Button
                            size="sm"
                            onClick={props.handleAssignTenantRole}
                            disabled={!props.shortcutTenantId().trim() || !props.shortcutTenantUserId().trim()}
                        >
                            Assign membership
                        </Button>
                        <Button
