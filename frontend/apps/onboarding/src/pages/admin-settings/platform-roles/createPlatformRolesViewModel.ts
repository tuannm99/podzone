...
export function createPlatformRolesViewModel(userID: number, enabled: Accessor<boolean>) {
    ...
    const roles = createPaginatedResource(
        ...
        {
            enabled: () => enabled() && Number.isFinite(selectedUserID()) && selectedUserID() > 0,
    ...
    const save = async (values: PlatformRoleFormValues) => {
        ...
        if (!Number.isFinite(nextUserID) || nextUserID <= 0) {
            setMutationError('Target user id is required for platform administration roles.')
            return
        }
        setSaving(true)
        setMutationError('')
        ...
        try {
            ...
            setMessage(`Saved platform admin role ${values.roleName} for user ${nextUserID}.`)
            if (selectedUserID() === nextUserID) {
                await roles.reload()
            } else {
                setSelectedUserID(nextUserID)
...
export type PlatformRolesViewModel = ReturnType<typeof createPlatformRolesViewModel>
