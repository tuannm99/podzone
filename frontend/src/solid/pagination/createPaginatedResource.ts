...
export function createPaginatedResource<T>(
    ...
    const [resource, { mutate, refetch }] = createResource<CollectionPage<T>, ResourceSource>(
        () =>
        ...
        async ({ query: currentQuery }, info): Promise<CollectionPage<T>> => {
            try {
                ...
                setReadError(error instanceof Error ? error.message : 'Failed to load collection')
                return (
                    info.value || {
                        items: [],
    ...
    if (options?.dependency) {
        createEffect(on(options.dependency, () => setQuery('page', 1), { defer: true }))
    }

    const items = () => resource.latest?.items || []
    const pageInfo = () => resource.latest?.pageInfo || emptyPageInfo({ ...query })
    const loading = () => resource.loading
    const resolved = () => !readError() && (resource.state === 'ready' || resource.state === 'refreshing')
