...
import type { IamResult, PolicyStatement, PolicyInfo, GroupInlinePolicy, GroupInfo, CreateGroupPayload } from './types'

export async function createGroup(payload: CreateGroupPayload): Promise<IamResult<{ group?: GroupInfo }>> {
    try {
        const { data } = await http.post<{ group?: GroupInfo }>('/auth/v1/iam/groups', payload)
...
export async function addGroupMember(groupId: number, userId: number): Promise<IamResult<true>> {
    try {
...
export async function removeGroupMember(groupId: number, userId: number): Promise<IamResult<true>> {
    try {
...
export async function attachGroupPolicy(groupId: number, policyName: string): Promise<IamResult<true>> {
    try {
...
export async function detachGroupPolicy(groupId: number, policyName: string): Promise<IamResult<true>> {
    try {
...
export async function deleteGroupInlinePolicy(groupId: number, name: string): Promise<IamResult<true>> {
    try {
