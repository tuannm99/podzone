...
export async function createRoutedOrder(payload: CreateRoutedOrderPayload): Promise<OrdersResult<RoutedOrder>> {
...
export async function advanceRoutedOrder(id: string): Promise<OrdersResult<RoutedOrder>> {
    const result = await postBackofficeGraphQL<{
...
export async function openRoutedOrderException(id: string, exceptionType: string): Promise<OrdersResult<RoutedOrder>> {
    const result = await postBackofficeGraphQL<{
...
export async function updateRoutedOrderExceptionStatus(id: string, status: string): Promise<OrdersResult<RoutedOrder>> {
    const result = await postBackofficeGraphQL<{
    ...
    `,
        {
            input: {
                ...
                operatorAssignee:
                    payload.operatorAssignee && payload.operatorAssignee.trim() ? payload.operatorAssignee : null,
                shipmentSlaDueAt: payload.shipmentSlaDueAt || null,
                settlementStatus:
                    payload.settlementStatus && payload.settlementStatus.trim() ? payload.settlementStatus : null,
            },
        }
    )
    if (!result.success) {
