...
export function useOrderStorage(params: OrderStorageParams) {
    ...
    const [savedBulkTemplates, setSavedBulkTemplates] = createSignal<SavedBulkTemplate[]>([])
    ...
    const loadSavedPresets = () => {
        const raw = window.localStorage.getItem(queuePresetStorageKey(params.tenantId(), params.storeId()))
        if (!raw) {
            setSavedPresets([])
            return
        }
    ...
    const persistSavedPresets = (next: SavedQueuePreset[]) => {
        window.localStorage.setItem(queuePresetStorageKey(params.tenantId(), params.storeId()), JSON.stringify(next))
        setSavedPresets(next)
    }

    const loadSavedBulkTemplates = () => {
        const raw = window.localStorage.getItem(bulkTemplateStorageKey(params.tenantId(), params.storeId()))
        if (!raw) {
    ...
    const persistSavedBulkTemplates = (next: SavedBulkTemplate[]) => {
        window.localStorage.setItem(bulkTemplateStorageKey(params.tenantId(), params.storeId()), JSON.stringify(next))
    ...
    const deleteBulkTemplate = (name: string) => {
        persistSavedBulkTemplates(savedBulkTemplates().filter((template) => template.name !== name))
    ...
    const applyRelativeShipmentSla = (mode: ShipmentSlaMode) => {
        setBulkDraft((current) => ({
            ...
            shipmentSlaDueAt: mode ? toLocalDateTimeValue(resolveShipmentSla(mode)) : current.shipmentSlaDueAt,
        }))
    }
