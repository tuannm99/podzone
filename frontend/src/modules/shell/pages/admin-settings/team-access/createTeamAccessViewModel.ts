...
export function createTeamAccessViewModel(userID: number, access: WorkspaceAccessViewModel) {
    ...
    const save = async (values: TeamMemberFormValues) => {
        ...
        if (!tenantID || (!memberIdentity && (!Number.isFinite(parsedUserID) || parsedUserID <= 0))) {
            setMutationError('Workspace id and either teammate identity or user id are required.')
            return
        }
        if (!access.canManage()) {
            setMutationError('You do not have permission to manage team access for this workspace.')
            return
        }
        ...
        try {
            if (memberIdentity) {
                ...
                setUserId('')
                setMessage(`Saved workspace access for user ${parsedUserID} in ${tenantID}.`)
            }
