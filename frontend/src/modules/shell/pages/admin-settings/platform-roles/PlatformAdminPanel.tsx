...
import { FormInputField, FormSelectField, createFormStore, numberValue, required } from '@podzone/shared/ui/forms'
...
export function PlatformAdminPanel() {
    ...
    const platformRoleForm = createFormStore<PlatformRoleFormValues>({
        ...
        validators: {
            userId: [required('Target user id is required.'), numberValue('Target user id must be a number.')],
    ...
    return (
        <Card class="space-y-4">
            ...
            <Show when={!platformRoles.loading() && !platformRoles.canManage()}>
                <InfoAlert>Platform administration requires dedicated platform access.</InfoAlert>
            </Show>
            <form class="space-y-4" onSubmit={submitPlatformRole}>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <Button type="submit" loading={platformRoles.saving()} disabled={!platformRoles.canManage()}>
                        Save admin role
                    </Button>
                    <Button
                        type="button"
