...
export default function DevAuthBootstrapPage() {
    ...
    const importBundle = async () => {
        ...
        try {
            ...
            if (!nextBundle.accessToken || !nextBundle.refreshToken || !nextBundle.tenantId) {
            ...
            setStatus('error')
            setBundle(null)
            setMessage(error instanceof Error ? error.message : 'Unable to import the dev auth bundle.')
        }
    }
                    ...
                    <Show when={status() === 'ready' && bundle()}>
                        {(resolved) => (
                            <div class="grid gap-2 rounded-lg bg-slate-50 p-4 text-sm text-slate-700">
                                <p>
                                    <span class="font-semibold text-slate-900">Tenant:</span> {resolved().tenantId}
                                </p>
                                <p>
                                    <span class="font-semibold text-slate-900">User:</span> {resolved().username}
                                </p>
                                <p>
                                    <span class="font-semibold text-slate-900">Email:</span> {resolved().email}
                                </p>
                            </div>
                        )}
                    </Show>
                    ...
                    <Show when={status() === 'error'}>
                        <div class="space-y-4">
                            ...
                            <div class="rounded-lg bg-slate-50 p-4 text-sm text-slate-700">
                                ...
                                <p class="mt-2">
                                    Run `make dev-pod-sample ...` first, then sync the generated bundle into the UI
                                    assets with `make dev-ui-auth-sync`.
                                </p>
                            </div>
                        </div>
                    </Show>
