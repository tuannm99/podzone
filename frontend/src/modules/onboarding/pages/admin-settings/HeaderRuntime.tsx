        ...
        <>
            <section class="flex flex-col gap-4 border-b border-gray-200 pb-5 lg:flex-row lg:items-center lg:justify-between">
                ...
                <Show when={!platformRoles.canManage()}>
                    <span class="self-center text-sm text-gray-500">Platform IAM is unavailable for this session.</span>
                </Show>
            </section>
...
function LocalSessionState() {
    ...
    return (
        <Card class="space-y-4">
            <SectionTitle title="Local session state" subtitle="Storage-backed sign-in and navigation state." />
            <div class="flex flex-wrap gap-2">
                ...
                />
                <Badge
                    content={user.sessionID() ? `session ${user.sessionID()}` : 'session missing'}
