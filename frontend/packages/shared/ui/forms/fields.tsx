...
export function FormTextareaField<TValues extends Record<string, unknown>>(
    ...
    return (
        <TextareaField
            ...
            errorText={props.form.error(props.name)}
            onInput={(event) =>
                props.form.setValue(props.name, event.currentTarget.value as TValues[typeof props.name])
            }
        />
    )
}

export function FormCheckboxField<TValues extends Record<string, unknown>>(props: FormFieldProps<TValues>) {
    return (
        <CheckboxField
            label={props.label}
            checked={Boolean(props.form.value(props.name))}
            onChange={(event) =>
                props.form.setValue(props.name, event.currentTarget.checked as TValues[typeof props.name])
            }
        />
    )
}
