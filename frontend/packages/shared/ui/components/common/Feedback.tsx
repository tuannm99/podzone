...
function AlertBox(props: ParentProps<{ color: 'failure' | 'warning' | 'info' }>) {
    ...
    return (
        <div class={classes('rounded-lg border px-4 py-3 text-sm shadow-sm', colorClasses[props.color])}>
            {props.children}
        </div>
    )
}
...
export function EmptyBlock(props: { title: string; copy: string }) {
    return (
        <div class="rounded-lg border border-dashed border-gray-200 bg-gray-50 px-6 py-10 text-center">
            <h3 class="text-base font-semibold text-gray-950">{props.title}</h3>
            <p class="mt-2 text-sm text-gray-600">{props.copy}</p>
        </div>
    )
}
