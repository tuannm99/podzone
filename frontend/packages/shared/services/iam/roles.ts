...
export async function putRoleTrustPolicy(payload: PutRoleTrustPolicyPayload): Promise<IamResult<true>> {
    try {
...
export async function getRoleTrustPolicy(roleName: string): Promise<IamResult<RoleTrustStatement[]>> {
    try {
...
export async function putRolePermissionBoundary(roleName: string, policyName: string): Promise<IamResult<true>> {
    try {
...
export async function getRolePermissionBoundary(roleName: string): Promise<IamResult<RolePermissionBoundary | null>> {
    try {
...
export async function deleteRolePermissionBoundary(roleName: string): Promise<IamResult<true>> {
    try {
...
export async function upsertPlatformRole(payload: UpsertPlatformRolePayload): Promise<IamResult<true>> {
    try {
        await http.post(`/auth/v1/iam/platform-users/${payload.targetUserId}/roles`, payload)
        return { success: true, data: true }
    } catch (error) {
...
export async function removePlatformRole(targetUserId: number, roleName: string): Promise<IamResult<true>> {
    try {
