...
export async function createOrganization(
    ...
    try {
        const { data } = await http.post<CreateOrganizationResult>('/auth/v1/iam/organizations', payload)
...
export async function addOrganizationMember(orgId: string, userId: number, roleName: string): Promise<IamResult<true>> {
    try {
...
export async function removeOrganizationMember(orgId: string, userId: string): Promise<IamResult<true>> {
    try {
...
export async function attachTenantToOrganization(orgId: string, tenantId: string): Promise<IamResult<true>> {
    try {
...
export async function detachTenantFromOrganization(orgId: string, tenantId: string): Promise<IamResult<true>> {
    try {
...
export async function attachServiceControlPolicy(orgId: string, policyName: string): Promise<IamResult<true>> {
    try {
        await http.post(`/auth/v1/iam/organizations/${orgId}/service-control-policies`, { orgId, policyName })
        return { success: true, data: true }
    } catch (error) {
...
export async function detachServiceControlPolicy(orgId: string, policyName: string): Promise<IamResult<true>> {
    try {
        await http.delete(`/auth/v1/iam/organizations/${orgId}/service-control-policies/${policyName}`)
        return { success: true, data: true }
    } catch (error) {
...
export async function listServiceControlPolicies(orgId: string): Promise<IamResult<PolicyInfo[]>> {
    try {
