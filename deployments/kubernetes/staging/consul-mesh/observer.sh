...
helm repo update && \
helm install --values ./prometheus.yml prometheus prometheus-community/prometheus --version "15.5.3" && \
kubectl rollout status deployment prometheus-server --namespace default --timeout=300s && \
helm install loki --values ./loki.yml grafana/loki-stack --version "2.9.9" && \
kubectl rollout status statefulset loki --namespace default --timeout=300s && \
helm install --values ./grafana.yml grafana grafana/grafana --version "6.23.1" && \
